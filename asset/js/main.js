...
$(document).ready(function(){
	...
	// sub_stay_page
	if($(".stay_house_view").length){

        var stay_sokcho = {
			lat : 38.1894183671324,
			long: 128.6027388863058
		}
		var stay_jeju = {
			lat : 33.25170162755272,
			long: 126.50996066949392
		}
		
		// if(page_name == "stay_sokcho"){
		// 	kakao_map(stay_sokcho.lat, stay_sokcho.long);
		// }else if(page_name == "stay_jeju"){
		// 	kakao_map(stay_jeju.lat, stay_jeju.long);
		// }
		...
		// swiper 변경시 이벤트
		space_swiper.on('onSlideChangeStart', function () {

            var slide_idx = space_swiper.activeIndex;
			bullets.removeClass("active");
			thumbs.removeClass("active");
		...
		function openModal2(modal_name) {
		...
		// 	const item = e.querySelector('img')
		// 	const target = document.querySelector('.view > .item')
		// 	const copyNode = item.cloneNode(true);
		// 	console.log(copyNode);
		// 	const thumbList = document.querySelectorAll('.thumb > a')
		...
		$('.stay_modal').click(function() {
			var id = $(this).attr("id");
			var modal_name = "#modal"+id.charAt(id.length-1);
			openModal2(modal_name);
		...
		$('.closeModal').click(function() {
			...
			$(this).parent().parent().children(':first-child').children().eq(1).children().attr('class',"");
			$(this).parent().parent().children(':first-child').children().eq(1).children(':first-child').attr('class',"ac");
			// var view_src = $('.conWrap .modal_thumb .ac').attr('src');
			// console.log(view_src);
			// $('.modal .imgWrap .modal_view img').attr("src",view_src);
			// $('.imgWrap .modal_thumb ac').attr('src');
			var firest_view_src = $(this).parent().parent().children(':first-child').children().eq(1).children(':first-child').children().attr('src');
			// console.log(firest_view_src);
			$(this).parent().parent().children(':first-child').children(':first-child').children().children().attr("src",firest_view_src);


		});
		
		$('.imgWrap .modal_thumb img').click(function() {
			var src = $(this).attr("src");
			// console.log(src);
			// console.log($(this).parent().parent().parent().children(':first-child').children().children());
			$(this).parent().parent().parent().children(':first-child').children().children().attr('src',src);
			// $('.imgWrap .modal_view img').attr('src',src);
			$(this).parent().parent().children().attr('class',"");
			$(this).closest('a').attr('class',"ac");
		});

		//채널톡 활성화 
		$('#cht_open').click(function() {
			ChannelIO('showMessenger');
		});
    }

	// 헤더메뉴 클릭 event (메인/서브 페이지 랜딩)
    $('.page_landing').off().on('click', function() {
    	...
    	if(target_name != 'main') {
        	...
        	urlParams.set('prev', page_name);

    		location.href = "/"+target_name+'?'+urlParams;

    	} else if(target_name == 'main') {
...
window.onload = function googleTranslateElementInit() {
	...
	console.log(width);
	if(width >768){ // 991 would be the width of the mobile break point.
		new google.translate.TranslateElement({pageLanguage: 'ko', autoDisplay: false}, 'google_translate_element_dk');
		console.log("dk");
	}else{
		new google.translate.TranslateElement({pageLanguage: 'ko', autoDisplay: false}, 'google_translate_element_mo');
		console.log("mo");
	}
}